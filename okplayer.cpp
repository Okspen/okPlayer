...
void okPlayer::setPlaylistWidget(okPlaylistWidget *list)
{
	...
	playlistWidget->insertColumn(0);
	playlistWidget->insertColumn(1);
...
void okPlayer::updateStats(int files, int total, int folders, QString time)
{
    emit statusChanged(QString("%1 (of %2) files in %3 folders for %4").arg(files).arg(total).arg(folders).arg(time));
}
...
void okPlayer::setPlayingModeState(int newState)
{
    ...
    switch(newState)
    {
    case 0: setPlayingModeState(StopAfterTrack);
            break;
    case 1: setPlayingModeState(StopAfterPlaylist);
            break;
    case 2: setPlayingModeState(LoopTrack);
            break;
    case 3: setPlayingModeState(LoopPlaylist);
            break;
    }
}
...
{
	playlistFromPath(fsModel->filePath(i));
}
...
void okPlayer::playlistFromPath(const QString& path, bool onlyRoot)
{
	...
	if(fInfo.isDir())
	{
		okHDDThread* hddThread = new okHDDThread(getFileExt(), append);
	...
	{
		qDebug() << path.lastIndexOf(".");
		qDebug() << path.right(path.length() - path.lastIndexOf(".")).toLower();
...
{
	okHDDThread* hddThread = (okHDDThread*) sender();
...
{
	playlistToFile(playlistWidget->getFavouriteTracks(), "favourites.m3u");
}
